...
// Lắng nghe sự kiện click trên nút "Add product" addCommodityProductButton
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById('addCommodityProductButton').addEventListener('click', addCommodityForm);
});

function addCommodityForm(event) {
    this.setAttribute('data-action', 'addCommodity');
...
cancelButton.setAttribute('data-action', 'addCommodity');

var closeButton = document.getElementById("closeAddProduct");
closeButton.setAttribute('data-action', 'addCommodity');
...
saveButton.setAttribute('data-action', 'addCommodity');

  
    // Thêm title tương ứng
    document.getElementById('titleAddProduct').innerText = 'Thêm hàng hóa';
    ...
    // // Thêm sự kiện hiển thị div "add-product" khi click vào button #addCommodityProductButton
    //   showDiv("addCommodityProductButton", ".add-product");

    // Thêm sự kiện hiển thị div "tab-content" khi click vào button #addCommodityProductButton
    showDiv("addCommodityProductButton", ".tab-content");
  ...
  saveButton.addEventListener("click", addCommodityButtonClickHandler); 

// Sự kiện click cho nút cancelAddProduct
cancelButton.addEventListener("click", cancelCommodityButtonClickHandler);
// Sự kiện click cho nút closeAddProduct
closeButton.addEventListener("click", closeCommodityButtonClickHandler);
...
function addCommodityButtonClickHandler() {
  if (action === 'addCommodity') {
    console.log("actionAddCommodity: ", action);
  ...
  if (!isValid) {
    ...
    console.log("Điều kiện của tồn tại validateAndFocusExistsFieldProduct() để tiếp tục thực thi isValid, validateAndFocusExistsFieldProduct(): !isExists: ", ifIsNotExists);
      ...
      onClickAddNewProductCommodity(); 
    }
  }
  } return;
  }

  function cancelCommodityButtonClickHandler() {
    hideAddProduct();
    console.log("actionAddCommodity: ", action);
    action = ''; // Gán giá trị rỗng cho action
    cancelButtonClicked = true;
    resetButtonClickedStatus(); // Đặt lại trạng thái của các biến
    }
    
    function closeCommodityButtonClickHandler() {
    hideAddProduct();
    console.log("actionAddCommodity: ", action);
    action = ''; // Gán giá trị rỗng cho action
...
function onClickAddNewProductCommodity() {
  // Sinh id cho product
  ...
  var type = "Hàng hóa"; // Trường hợp này đang thêm mới hàng hóa
  console.log(type);
  ...
  // Kiểm tra xem mã hàng hóa product.code có phải null hay rỗng không?
  ...
  // Sinh mã động cho hàng hóa dạng SP000001
  // Kiểm tra xem mã hàng hóa product.code có phải null hay rỗng không?
  if (productAdd.code === null || productAdd.code === "") {
    var prefix = "SP"; // Tiền tố cố định cho mã hàng hóa
    var digitCount = 6; // Số lượng chữ số trong mã hàng hóa

    // Lọc ra các hàng hóa có mã bắt đầu bằng 'SP'
    var productListSP = productList.filter(function (product) {
      // return product.code.startsWith("SP");
      return /^SP\d+$/.test(product.code) && 
      !/^SP0*$/.test(product.code); // Lọc ra các hàng hóa có mã bắt đầu bằng 'SP' và theo sau là ít nhất một chữ số, loại bỏ trường hợp toàn số 0
    });

    // Sắp xếp danh sách hàng hóa theo thứ tự tăng dần của mã hàng hóa trong mảng productListSP
    productListSP.sort(function (a, b) {
      var codeA = parseInt(a.code.substring(2)); // Lấy số từ mã hàng hóa
    ...
    if (
      ...
      // Duyệt qua danh sách hàng hóa đã được sắp xếp để tìm mã hàng hóa mới
      for (var i = 0; i < productListSP.length - 1; i++) {
        ...
        // Nếu có khoảng trống giữa các số hàng hóa
      ...
      // Nếu không tìm thấy khoảng trống, tạo ra mã hàng hóa mới bằng cách tăng mã hàng hóa lớn nhất hiện có thêm một đơn vị
      if (eCodeCurrent === null) {
        ...
        // Kiểm tra xem mã hàng hóa mới có trùng với bất kỳ mã nào trong danh sách không
  ...
  console.log("Hình ảnh của hàng hóa vừa nhập vào là: " + productAdd.img);

  console.log("Mã của hàng hóa là: " + productAdd.code);

  console.log("Tên của hàng hóa vừa nhập vào là: " + productAdd.name);

  console.log(
    "Mã vạch của hàng hóa vừa nhập vào là: " + productAdd.barcode
  );

  console.log("Thương hiệu của hàng hóa vừa nhập vào là: " + productAdd.trademark);

  console.log("ID của hàng hóa vừa nhập vào là: " + productAdd.id);

  /* 3. Dua product vao danh sach */
  productList.push(productAdd);
  console.log("Danh sách các hàng hóa sau khi thêm mới là: ");
  ...
  console.log("productId ở thao tác hiện tại sau khi click button btnSaveApplicationProduct trong quá trình thêm hàng hóa là: ", productId);
  // location.reload();
  closeButtonId('btnSaveApplicationProduct', 'addProduct');
};