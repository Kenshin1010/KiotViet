...
// Lắng nghe sự kiện click trên nút "Add product" addServiceProductButton
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById('addServiceProductButton').addEventListener('click', addServiceForm);
});

 function addServiceForm(event) {
    this.setAttribute('data-action', 'addService');
  ...
  cancelButton.setAttribute('data-action', 'addService');

  var closeButton = document.getElementById("closeAddProduct");
  closeButton.setAttribute('data-action', 'addService');
  ...
  saveButton.setAttribute('data-action', 'addService');

  
    // Thêm title tương ứng
    document.getElementById('titleAddProduct').innerText = 'Thêm dịch vụ';
    ...
    focusOnInput("productCode");
      ...
      //   // Thêm sự kiện hiển thị div "add-product" khi click vào button #addServiceProductButton
      // showDiv("addServiceProductButton", ".add-product");
    
      // Thêm sự kiện hiển thị div "tab-content" khi click vào button #addServiceProductButton
      showDiv("addServiceProductButton", ".tab-content");
  ...
  saveButton.addEventListener("click", addServiceButtonClickHandler);

  // Sự kiện click cho nút cancelAddProduct
cancelButton.addEventListener("click", cancelServiceButtonClickHandler);
// Sự kiện click cho nút closeAddProduct
closeButton.addEventListener("click", closeServiceButtonClickHandler);
...
function addServiceButtonClickHandler() {
  ...
  if (!isValid) {
    ...
    console.log("Điều kiện của tồn tại validateAndFocusExistsFieldProduct() để tiếp tục thực thi isValid, validateAndFocusExistsFieldProduct(): !isExists: ", ifIsNotExists);
      ...
      onClickAddNewProductService(); 
    }
  }
} return;
  }

  function cancelServiceButtonClickHandler() {
    hideAddProduct();
    console.log("actionAddService: ", action);
    ...
    function closeServiceButtonClickHandler() {
    hideAddProduct();
    console.log("actionAddService: ", action);
    action = ''; // Gán giá trị rỗng cho action
...
function onClickAddNewProductService() {
  // Sinh id cho product
  ...
  var type = "Dịch vụ"; // Trường hợp này đang thêm mới dịch vụ
  console.log(type);
  ...
  if (productAdd.code === null || productAdd.code === "") {
    ...
    var productListSP = productList.filter(function (product) {
      ...
      !/^SP0*$/.test(product.code); // Lọc ra các dịch vụ có mã bắt đầu bằng 'SP' và theo sau là ít nhất một chữ số, loại bỏ trường hợp toàn số 0
    });
  ...
  console.log("Hình ảnh của dịch vụ vừa nhập vào là: " + productAdd.img);

  console.log("Mã của dịch vụ là: " + productAdd.code);

  console.log("Tên của dịch vụ vừa nhập vào là: " + productAdd.name);

  console.log(
    "Mã vạch của dịch vụ vừa nhập vào là: " + productAdd.barcode
  );

  console.log("Thương hiệu của dịch vụ vừa nhập vào là: " + productAdd.trademark);

  console.log("ID của dịch vụ vừa nhập vào là: " + productAdd.id);

  /* 3. Dua product vao danh sach */
  productList.push(productAdd);
  console.log("Danh sách các dịch vụ sau khi thêm mới là: ");
  ...
  console.log("productId ở thao tác hiện tại sau khi click button btnSaveApplicationProduct trong quá trình thêm dịch vụ là: ", productId);
  // location.reload();
  closeButtonId('btnSaveApplicationProduct', 'addProduct');
};
...
// Để tránh lỗi DOM
// document.addEventListener("DOMContentLoaded", function () {
//   // Xác định sự kiện cho nút #addServiceProductButton
//   document.getElementById('addServiceProductButton').addEventListener('click', function() {
//       // Cập nhật giá trị data-action cho nút #btnSaveApplicationProduct
//       document.getElementById('btnSaveApplicationProduct').setAttribute('data-action', 'addService');
//       // Xác định sự kiện cho nút #btnSaveApplicationProduct
//       document.getElementById('btnSaveApplicationProduct').addEventListener('click', function() {
//           // Xử lý sự kiện lưu sản phẩm theo cách thêm mới "Dịch vụ"
//           // ...
//       });
//   });

//   // Tương tự cho các nút #addServiceProductButton và #addComboProductButton
// });
