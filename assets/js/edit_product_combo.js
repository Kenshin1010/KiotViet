...
document.querySelectorAll(".btn-edit-product-combo").forEach(function (button) {
  button.addEventListener("click", editComboForm);
});
function editComboForm() {
    // toggleDisplay(button.getAttribute('id'), '.add-product', 'productName');
    this.setAttribute('data-action', 'editCombo');
    action = this.getAttribute('data-action');
    console.log(action);
    
    var cancelButton = document.getElementById("cancelAddProduct");
    cancelButton.setAttribute('data-action', 'editCombo');

    var closeButton = document.getElementById("closeAddProduct");
    closeButton.setAttribute('data-action', 'editCombo');
    ...
    saveButton.setAttribute('data-action', 'editCombo');
  ...
  document.getElementById('titleAddProduct').innerText = 'Sửa combo - đóng gói';
    ...
    // Gọi hàm fillProductData để điền dữ liệu của combo-đóng gói vào form
    var editedProduct = fillProductData(productId);
    console.log("editedProduct: ", editedProduct);
  
    // Lấy thông tin combo-đóng gói đang chỉnh sửa
    // var productEditing = getProductById(productId);
    // console.log("productEditing: ", productEditing);
  
    // Lấy danh sách combo-đóng gói
    var productList = getProductListLocalStorage();
  
    // Tạo danh sách combo-đóng gói không chứa combo-đóng gói đang chỉnh sửa
    ...
    document
      ...
      saveButton.addEventListener("click", editComboButtonClickHandler);
      
    // Sự kiện click cho nút cancelAddProduct
cancelButton.addEventListener("click", cancelEditComboButtonClickHandler);
// Sự kiện click cho nút closeAddProduct
closeButton.addEventListener("click", closeEditComboButtonClickHandler);
...
// }
  ...
  function editComboButtonClickHandler() {
    if (action === "editCombo") {
    ...
    if (!isValid) {
      ...
      if (!ifIsNotExistsEdit) {
        ...
        // Gọi hàm để khởi tạo
        // Nếu productId đã tồn tại, gọi hàm updateProductComboInList
        ...
        if (codeInput === null || codeInput === "") {
          ...
          var productListSP = productList.filter(function (product) {
            ...
            ); // Lọc ra các combo-đóng gói có mã bắt đầu bằng 'SP' và theo sau là ít nhất một chữ số, loại bỏ trường hợp toàn số 0
          });
        ...
        var productTypeCombo = "Combo-đóng gói";

        // Tạo một đối tượng mới chứa thông tin chỉnh sửa của combo-đóng gói
        var editedProduct = {
        ...
        updateProductComboInList(productId, editedProduct);
  ...
  function cancelEditComboButtonClickHandler() {
    hideEditProduct();
  ...
  function closeEditComboButtonClickHandler() {
    hideEditProduct();
  console.log("actionAddService: ", action);
  action = ''; // Gán giá trị rỗng cho action
...
function fillProductData(productId) {
  // Lấy danh sách combo-đóng gói từ localStorage
  var productList = JSON.parse(localStorage.getItem("productList"));

  // Tìm combo-đóng gói cần chỉnh sửa trong danh sách combo-đóng gói
  var editedProduct = productList.find(function (product) {
    return product.id === productId;
  });

  // Điền dữ liệu của combo-đóng gói vào các trường của form chỉnh sửa
  ...
  var productTypeComboEdited = "Combo-đóng gói";
  productTypeComboEdited = editedProduct.type;
  // Trả về đối tượng combo-đóng gói đã chỉnh sửa
  return editedProduct;
}

function updateProductComboInList(productId, editedProduct) {
  console.log("Goi ham updateProductComboInList()");

  // Lấy danh sách combo-đóng gói từ Local Storage
  var productList = JSON.parse(localStorage.getItem("productList"));

  // Tìm combo-đóng gói có ID tương ứng và cập nhật thông tin
  ...
  console.log("Combo-đóng gói được sửa có mã hàng là: ", editedProduct.code);
  ...
  // alert("Sửa combo-đóng gói thành công");
  toast({
    type: "success",
    message: "Sửa combo-đóng gói thành công",
    duration: 1000,
  });
