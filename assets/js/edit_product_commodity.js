...
document.querySelectorAll(".btn-edit-product").forEach(function (button) {
  button.addEventListener("click", editCommodityForm);
});
function editCommodityForm() {
    // toggleDisplay(button.getAttribute('id'), '.add-product', 'productName');
    this.setAttribute('data-action', 'editCommodity');
    action = this.getAttribute('data-action');
    console.log(action);

var cancelButton = document.getElementById("cancelAddProduct");
cancelButton.setAttribute('data-action', 'editCommodity');

var closeButton = document.getElementById("closeAddProduct");
closeButton.setAttribute('data-action', 'editCommodity');
...
saveButton.setAttribute('data-action', 'editCommodity');
    ...
    document.getElementById('titleAddProduct').innerText = 'Sủa hàng';
    ...
    // Gọi hàm fillProductData để điền dữ liệu của hàng hóa vào form
    var editedProduct = fillProductData(productId);
    console.log("editedProduct: ", editedProduct);
  
    // Lấy thông tin hàng hóa đang chỉnh sửa
    ...
    // Tạo danh sách hàng hóa không chứa hàng hóa đang chỉnh sửa
    ...
    cancelButton.addEventListener("click", cancelEditCommodityButtonClickHandler);
    // Sự kiện click cho nút closeAddProduct
      closeButton.addEventListener("click", closeEditCommodityButtonClickHandler);
...
// }
  ...
  function editCommodityButtonClickHandler() {
    if (action === "editCommodity") {
    ...
    if (!isValid) {
      ...
      if (!ifIsNotExistsEdit) {
        ...
        // Gọi hàm để khởi tạo
        // Nếu productId đã tồn tại, gọi hàm updateProductCommodityInList
        ...
        if (codeInput === null || codeInput === "") {
          ...
          var productListSP = productList.filter(function (product) {
            ...
            ); // Lọc ra các hàng hóa có mã bắt đầu bằng 'SP' và theo sau là ít nhất một chữ số, loại bỏ trường hợp toàn số 0
          });
        ...
        var productTypeCommodity = "Hàng hóa";

        // Tạo một đối tượng mới chứa thông tin chỉnh sửa của hàng hóa
        var editedProduct = {
          ...
          type: productTypeCommodity,
        };

        updateProductCommodityInList(productId, editedProduct);
  ...
  function cancelEditCommodityButtonClickHandler() {
    hideEditProduct();
...
function closeEditCommodityButtonClickHandler() {
  hideEditProduct();
console.log("actionAdd: ", action);
action = ''; // Gán giá trị rỗng cho action
...
function fillProductData(productId) {
  // Lấy danh sách hàng hóa từ localStorage
  var productList = JSON.parse(localStorage.getItem("productList"));

  // Tìm hàng hóa cần chỉnh sửa trong danh sách hàng hóa
  var editedProduct = productList.find(function (product) {
    return product.id === productId;
  });

  // Điền dữ liệu của hàng hóa vào các trường của form chỉnh sửa
  ...
  var productTypeCommodity = "Hàng hóa";
  productTypeCommodity = editedProduct.type;
  // Trả về đối tượng hàng hóa đã chỉnh sửa
  return editedProduct;
}

function updateProductCommodityInList(productId, editedProduct) {
  console.log("Goi ham updateProductCommodityInList()");

  // Lấy danh sách hàng hóa từ Local Storage
  var productList = JSON.parse(localStorage.getItem("productList"));

  // Tìm hàng hóa có ID tương ứng và cập nhật thông tin
  ...
  console.log("Hàng hóa được sửa có mã hàng hóa là: ", editedProduct.code);
